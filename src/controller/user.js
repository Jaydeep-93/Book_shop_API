...
exports.login = async (req, res, next) => {
  try {
    ...
    if (users.length != 0) {
      ...
      const user = users[0];
      // convert gievn password in hash
      const hashPassword = await encryptPassword.hasMyPassword(password);
      // compare if password match
      if (encryptPassword.passwordMatch(hashPassword, user.password)) {
